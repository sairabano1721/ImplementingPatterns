...
@Repository
public class FacilityDAO implements IFacilityDAO,IFacilityVisitor {
	...
	@Override
	public Boolean getIsFacilityAvailablw(Integer facilityId) {
		List<Integer> allFacilities = new ArrayList<Integer>();
		Boolean status = true;
		try {
			dbHelper = getDBDAOObject(dbHelper);
			Statement stmt = dbHelper.getConnection().createStatement();
			String query = "select isAvailable from sql3395846.facilities where idFacilities = " + facilityId;
			;
			ResultSet rs = stmt.executeQuery(query);
			while (rs.next())
	...
	@Override
	public void updateFacility(Boolean available, Integer facilityId) {

		try {
			dbHelper = getDBDAOObject(dbHelper);
			Statement stmt = dbHelper.getConnection().createStatement();
			String query = "UPDATE sql3395846.facilities " + "SET isAvailable = " + available + " WHERE idFacilities = "
					+ facilityId;
			stmt.executeUpdate(query);
			System.out.println("Facility " + facilityId + "'s list of availablity has been updated");
		} catch (Exception e) {
			System.out.println(e.toString());
		}
